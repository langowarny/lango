...
func (_q *SecretQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Secret, error) {
	...
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, secret.ForeignKeys...)
	}
	...
	if query := _q.withKey; query != nil {
		if err := _q.loadKey(ctx, query, nodes, nil,
			func(n *Secret, e *Key) { n.Edges.Key = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SecretQuery) loadKey(ctx context.Context, query *KeyQuery, nodes []*Secret, init func(*Secret), assign func(*Secret, *Key)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Secret)
	for i := range nodes {
		if nodes[i].key_secrets == nil {
			continue
		}
		fk := *nodes[i].key_secrets
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "key_secrets" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *SecretQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(secret.Table, secret.Columns, sqlgraph.NewFieldSpec(secret.FieldID, field.TypeUUID))
	_spec.From = _q.sql
