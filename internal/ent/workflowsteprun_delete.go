// Code generated by ent, DO NOT EDIT.

package ent

import (
	"context"

	"entgo.io/ent/dialect/sql"
	"entgo.io/ent/dialect/sql/sqlgraph"
	"entgo.io/ent/schema/field"
	"github.com/langowarny/lango/internal/ent/predicate"
	"github.com/langowarny/lango/internal/ent/workflowsteprun"
)

// WorkflowStepRunDelete is the builder for deleting a WorkflowStepRun entity.
type WorkflowStepRunDelete struct {
	config
	hooks    []Hook
	mutation *WorkflowStepRunMutation
}

// Where appends a list predicates to the WorkflowStepRunDelete builder.
func (_d *WorkflowStepRunDelete) Where(ps ...predicate.WorkflowStepRun) *WorkflowStepRunDelete {
	_d.mutation.Where(ps...)
	return _d
}

// Exec executes the deletion query and returns how many vertices were deleted.
func (_d *WorkflowStepRunDelete) Exec(ctx context.Context) (int, error) {
	return withHooks(ctx, _d.sqlExec, _d.mutation, _d.hooks)
}

// ExecX is like Exec, but panics if an error occurs.
func (_d *WorkflowStepRunDelete) ExecX(ctx context.Context) int {
	n, err := _d.Exec(ctx)
	if err != nil {
		panic(err)
	}
	return n
}

func (_d *WorkflowStepRunDelete) sqlExec(ctx context.Context) (int, error) {
	_spec := sqlgraph.NewDeleteSpec(workflowsteprun.Table, sqlgraph.NewFieldSpec(workflowsteprun.FieldID, field.TypeUUID))
	if ps := _d.mutation.predicates; len(ps) > 0 {
		_spec.Predicate = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector)
			}
		}
	}
	affected, err := sqlgraph.DeleteNodes(ctx, _d.driver, _spec)
	if err != nil && sqlgraph.IsConstraintError(err) {
		err = &ConstraintError{msg: err.Error(), wrap: err}
	}
	_d.mutation.done = true
	return affected, err
}

// WorkflowStepRunDeleteOne is the builder for deleting a single WorkflowStepRun entity.
type WorkflowStepRunDeleteOne struct {
	_d *WorkflowStepRunDelete
}

// Where appends a list predicates to the WorkflowStepRunDelete builder.
func (_d *WorkflowStepRunDeleteOne) Where(ps ...predicate.WorkflowStepRun) *WorkflowStepRunDeleteOne {
	_d._d.mutation.Where(ps...)
	return _d
}

// Exec executes the deletion query.
func (_d *WorkflowStepRunDeleteOne) Exec(ctx context.Context) error {
	n, err := _d._d.Exec(ctx)
	switch {
	case err != nil:
		return err
	case n == 0:
		return &NotFoundError{workflowsteprun.Label}
	default:
		return nil
	}
}

// ExecX is like Exec, but panics if an error occurs.
func (_d *WorkflowStepRunDeleteOne) ExecX(ctx context.Context) {
	if err := _d.Exec(ctx); err != nil {
		panic(err)
	}
}
