...
The Automation Approval Routing capability provides context-based approval target override for routing tool approval requests from automation systems (cron, background) to the originating channel. This allows non-interactive automation sessions to request tool approval through the user's original channel (Telegram, Discord, Slack) instead of failing on TTY fallback.
