...
Sub-agent prompt inheritance and per-agent customization for multi-agent mode. Ensures all sub-agents inherit shared prompt sections (Safety, ConversationRules) and supports per-agent overrides via directory structure.
