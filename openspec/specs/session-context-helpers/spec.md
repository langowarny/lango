...
Shared session key context injection and extraction utilities in `internal/session`. Provides the canonical `WithSessionKey`/`SessionKeyFromContext` helpers used across `app`, `gateway`, and channel packages for context-based session key propagation.
